...
<div class="content-wrapper">
    ...
    @push('cjs')
        <script src="{{ asset('contents/admin/plugins/select2/js/select2.min.js') }}"></script>
        ...
        <script>
            ...
            $('#selectproduct_main_category_id').on('change',function(){
                let value = $(this).val();
                $.get("/admin/product/get-all-category-selected-by-main-category/"+value,(res)=>{
                    $('#selectproduct_category_id').html(res);
                })
            })
        </script>

        <script>
         $(document).on('select2:select select2:unselect', '.multiple-select', function(){
            $(this).trigger('change');
        });
    </script>
    @endpush
